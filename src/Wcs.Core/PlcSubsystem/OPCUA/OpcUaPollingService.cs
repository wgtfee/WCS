...
public class OpcUaPollingService : IDisposable
{
    ...
    public void Start()
    {
        ...
        foreach (var reg in _registrations)
        {
            ...
            if (instance == null)
            {
                _logger?.LogError("[OpcUaPoll] 无法实例化 {Type}", reg.StructType.Name);
                continue;
            }
        ...
        _logger?.LogInformation("[OpcUaPoll] 启动完成，共 {Count} 个类型", _loops.Count);
    }

    private async Task RunPollLoopAsync(OpcUaPollRegistration reg, object instance, CancellationToken cancellationToken)
    {
        try
        {
