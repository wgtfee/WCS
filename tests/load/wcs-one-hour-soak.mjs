...
const config = {
  ...
  durationSeconds: intFromEnv("DURATION_SECONDS", 3600),
  httpConcurrency: intFromEnv("HTTP_CONCURRENCY", 24),
  signalRConnections: intFromEnv("SIGNALR_CONNECTIONS", 100, 0),
  writeIntervalMs: intFromEnv("WRITE_INTERVAL_MS", 1000),
  requestTimeoutMs: intFromEnv("REQUEST_TIMEOUT_MS", 5000),
  maximumErrorRatePercent: numberFromEnv("MAX_ERROR_RATE_PERCENT", 0),
  maximumP95Milliseconds: numberFromEnv("MAX_P95_MS", 500),
  minimumSignalRMessages: intFromEnv("MIN_SIGNALR_MESSAGES", 1, 0),
  hostProcessId: intFromEnv("HOST_PID", 0, 0),
  outputPath: process.env.OUTPUT_PATH ?? "artifacts/one-hour-soak-results.json"
};
...
if (!loopbackHosts.has(targetHost) && process.env.ALLOW_REMOTE_TARGET !== "true") {
  throw new Error(`Refusing to load-test non-loopback target ${targetHost}`);
}
...
// Fixed-size histogram prevents the load generator from retaining millions of latency values.
const latencyHistogram = new Uint32Array(config.requestTimeoutMs + 2);
let latencyCount = 0;
...
function endpointStats(label) {
  if (!endpointResults.has(label)) {
    endpointResults.set(label, { requests: 0, failures: 0, statusCodes: {} });
  }
  return endpointResults.get(label);
}

function recordLatency(elapsedMilliseconds) {
  latencyCount++;
  maximumLatencyMs = Math.max(maximumLatencyMs, elapsedMilliseconds);
  const bucket = Math.max(
    0,
    Math.min(latencyHistogram.length - 1, Math.ceil(elapsedMilliseconds)));
  latencyHistogram[bucket]++;
}

function latencyPercentile(percentileValue) {
  if (latencyCount === 0) return 0;
  const target = Math.max(1, Math.ceil(latencyCount * percentileValue));
  let cumulative = 0;
  for (let index = 0; index < latencyHistogram.length; index++) {
...
function parseJsonBody(result) {
  if (!result.ok) throw new Error(`HTTP ${result.status ?? "network-error"}`);
  return JSON.parse(new TextDecoder().decode(result.body));
}

function readHostRssMb() {
  if (config.hostProcessId <= 0) return null;
  try {
...
function sampleMemory() {
  ...
  const elapsedSeconds = loadStartedAt > 0
    ? Math.max(0, (performance.now() - loadStartedAt) / 1000)
    : 0;
  memory.samples++;
  memory.initialRssMb ??= rssMb;
  memory.finalRssMb = rssMb;
  memory.peakRssMb = memory.peakRssMb === null ? rssMb : Math.max(memory.peakRssMb, rssMb);
...
function memoryTrendSummary() {
  const duration = Math.max(1, config.durationSeconds);
  const quarters = [0, 1, 2, 3].map(index => {
    const start = duration * index / 4;
    const end = duration * (index + 1) / 4;
    const values = memory.timeline
      .filter(sample => sample.elapsedSeconds >= start && sample.elapsedSeconds < end)
      .map(sample => sample.rssMb);
    const value = average(values);
    return value === null ? null : Number(value.toFixed(2));
  });

  const slopeWindowStart = Math.max(0, duration - 900);
  const slopeSamples = memory.timeline.filter(sample => sample.elapsedSeconds >= slopeWindowStart);
  let slopeMbPerMinute = null;
  if (slopeSamples.length >= 2) {
    const meanX = average(slopeSamples.map(sample => sample.elapsedSeconds));
    const meanY = average(slopeSamples.map(sample => sample.rssMb));
    let numerator = 0;
    let denominator = 0;
    for (const sample of slopeSamples) {
      const dx = sample.elapsedSeconds - meanX;
      numerator += dx * (sample.rssMb - meanY);
      denominator += dx * dx;
    }
    if (denominator > 0) slopeMbPerMinute = Number((numerator / denominator * 60).toFixed(3));
  }

  const q2 = quarters[1];
  const q4 = quarters[3];
  return {
    quarterAverageRssMb: quarters,
    q4MinusQ2Mb: q2 === null || q4 === null ? null : Number((q4 - q2).toFixed(2)),
    last15MinuteSlopeMbPerMinute: slopeMbPerMinute
  };
}
...
async function openSignalRConnection(index) {
  const negotiation = await fetch(new URL("/wcs/negotiate?negotiateVersion=1", config.baseUrl), {
    method: "POST",
    signal: AbortSignal.timeout(config.requestTimeoutMs)
  });
  if (!negotiation.ok) throw new Error(`SignalR negotiate returned HTTP ${negotiation.status}`);
  ...
  await new Promise((resolve, reject) => {
    const timeout = setTimeout(() => reject(new Error("SignalR handshake timed out")), config.requestTimeoutMs);
    ...
    socket.addEventListener("error", () => fail(new Error("SignalR WebSocket transport failed")));
    socket.addEventListener("close", event => {
      if (!subscriptionCompleted) {
        fail(new Error(`SignalR closed before subscription completed (${event.code})`));
      } else if (!closingSockets) {
        signalR.unexpectedCloses++;
        signalR.errors.push(`SignalR connection ${index} closed unexpectedly (${event.code})`);
      }
    });
    socket.addEventListener("message", async event => {
      ...
      for (const frame of frames) {
        ...
        try {
          ...
          fail(new Error(`SignalR returned invalid JSON: ${error instanceof Error ? error.message : String(error)}`));
          return;
        }
...
async function openSignalRConnections() {
  ...
  for (let start = 0; start < config.signalRConnections; start += batchSize) {
    ...
    const results = await Promise.allSettled(
      Array.from({ length: count }, (_, offset) => openSignalRConnection(start + offset)));
    for (const result of results) {
      if (result.status === "fulfilled") sockets.push(result.value);
      else signalR.errors.push(result.reason instanceof Error ? result.reason.message : String(result.reason));
    }
  }
}
  ...
  while (Date.now() < endsAt) {
    ...
    await measuredFetch("/api/tasks", {
      ...
      body: JSON.stringify({
        ...
        priority: 2,
        parameters: { source: "one-hour-soak", sequence }
      })
    }, "POST /api/tasks");
...
async function collectValidation() {
  ...
  const persistedDevices = parseJsonBody(await measuredFetch("/api/devices/db", {}, "validation /api/devices/db"));
  const persistedTasks = parseJsonBody(await measuredFetch("/api/tasks/db", {}, "validation /api/tasks/db"));
...
function buildResult() {
  ...
  const errorRatePercent = totalRequests === 0 ? 100 : failedRequests * 100 / totalRequests;
  const trend = memoryTrendSummary();
  return {
    ...
    http: {
      ...
      p50Milliseconds: latencyPercentile(0.50),
      p95Milliseconds: latencyPercentile(0.95),
      p99Milliseconds: latencyPercentile(0.99),
      maximumMilliseconds: Number(maximumLatencyMs.toFixed(2)),
...
async function main() {
  ...
  try {
    ...
    await Promise.all([
      ...Array.from({ length: config.httpConcurrency }, (_, index) => runReadWorker(index, endsAt)),
    ...
    if (loadStartedAt === 0) loadStartedAt = performance.now();
    loadCompletedAt = performance.now();
  } finally {
    if (memoryTimer) clearInterval(memoryTimer);
    closingSockets = true;
    for (const socket of sockets) {
      try { socket.close(1000, "load test completed"); } catch { }
    }
  }

  const errorRatePercent = totalRequests === 0 ? 100 : failedRequests * 100 / totalRequests;
  const p95Milliseconds = latencyPercentile(0.95);
  if (totalRequests < config.httpConcurrency) failures.push(`Only ${totalRequests} HTTP requests completed`);
  if (errorRatePercent > config.maximumErrorRatePercent) {
    failures.push(`HTTP error rate ${errorRatePercent.toFixed(3)}% exceeds ${config.maximumErrorRatePercent}%`);
  }
  if (p95Milliseconds > config.maximumP95Milliseconds) {
    failures.push(`HTTP P95 ${p95Milliseconds.toFixed(2)}ms exceeds ${config.maximumP95Milliseconds}ms`);
  }
  if (signalR.connectedConnections !== config.signalRConnections) {
    failures.push(`SignalR connected ${signalR.connectedConnections}/${config.signalRConnections}`);
  }
  if (signalR.serverMessages < config.minimumSignalRMessages) {
    failures.push(`SignalR received ${signalR.serverMessages} messages; expected at least ${config.minimumSignalRMessages}`);
  }
  if (signalR.unexpectedCloses > 0 || signalR.errors.length > 0) {
    failures.push(`SignalR recorded ${signalR.unexpectedCloses} unexpected closes and ${signalR.errors.length} errors`);
  }
  if ((validation.liveDeviceCount ?? 0) === 0) failures.push("Simulated PLC produced no live device state");
  if ((validation.persistedDeviceCount ?? 0) === 0) failures.push("SQL contains no persisted device state");
  if ((validation.persistedTaskCount ?? 0) === 0) failures.push("SQL contains no completed task evidence");
  if (validation.hostAliveAfterLoad !== true) failures.push("Host liveness check failed after load");
  const taskWrites = endpointResults.get("POST /api/tasks");
  if (!taskWrites || taskWrites.requests === 0 || taskWrites.failures > 0) {
    failures.push("Task write endpoint did not remain successful throughout the load");
  }
  ...
  console.log(JSON.stringify({
    ...
    memory: {
      ...
      initialRssMb: result.memory.initialRssMb,
      finalRssMb: result.memory.finalRssMb,
      peakRssMb: result.memory.peakRssMb,
      quarterAverageRssMb: result.memory.quarterAverageRssMb,
      q4MinusQ2Mb: result.memory.q4MinusQ2Mb,
      last15MinuteSlopeMbPerMinute: result.memory.last15MinuteSlopeMbPerMinute
    },
